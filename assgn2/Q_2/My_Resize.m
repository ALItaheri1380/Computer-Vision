function [Image] = My_Resize(Input_Image, Resizing_Factor)
    ...
    for i = 1:size(img_1,1)
        for j = 1:size(img_1,2)
            ...
            ax = max(min(floor((i-1)/Resizing_Factor) + 1, size(Input_Image,1)), 1);
            ay = max(min(floor((j-1)/Resizing_Factor) + 1, size(Input_Image,2)), 1);
            bx = max(min(floor((i-1)/Resizing_Factor) + 1, size(Input_Image,1)), 1);
            by = max(min(ceil((j-1)/Resizing_Factor) + 1, size(Input_Image,2)), 1);
            cx = max(min(ceil((i-1)/Resizing_Factor) + 1, size(Input_Image,1)), 1);
            cy = max(min(floor((j-1)/Resizing_Factor) + 1, size(Input_Image,2)), 1);
            dx = max(min(ceil((i-1)/Resizing_Factor) + 1, size(Input_Image,1)), 1);
            dy = max(min(ceil((j-1)/Resizing_Factor) + 1, size(Input_Image,2)), 1);
